$(document).ready(function () {
    ...
    $("input.business_case-name").on("change", function () {
        var text = $(this).val();
        var lang = $(this).attr("lang");
        var url = $("input.business_case-url[lang='" + lang + "']");
    ...
    $(".btn-lang").on("click", function () {
        ...
        if (_orig.length > 0) {
            target_langs.forEach(async function (lang) {
                ...
                if (targetClass !== "business_case-name") tinymce.get(lang + "-" + targetClass).setContent(translated);
            });
        }
    });
});